...
describe("Companies/Index", () => {
    ...
    it("triggers a debounced api refresh when the search term changes", async () => {
        ...
        await wrapper
            .get(`input[placeholder="${hu["companies.search_placeholder"]}"]`)
            .setValue("Beta");
        ...
        expect(listCompaniesMock).toHaveBeenLastCalledWith(
            companiesApi,
            expect.objectContaining({
                search: "Beta",
            }),
    ...
    it("updates a company and refreshes the list after save", async () => {
        ...
        updateCompanyMock.mockImplementationOnce(
            async (_api, _companyId, payload) => {
                submittedPayloads.push({ ...payload });
