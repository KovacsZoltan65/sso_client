            ...
            setup(props) {
                return () =>
                    h(
                        "div",
                        {},
                        (props.items ?? []).filter(Boolean).map((item) =>
                            h(
                                "button",
                                {
                                    type: "button",
...
describe("Employees/Index", () => {
    ...
    it("keeps the create flow working after the dialog fix", async () => {
        ...
        expect(submittedCreatePayloads[0]).toEqual(
            expect.objectContaining({
                company_id: 1,
                employee_number: "EMP-9",
                name: "New Hire",
    ...
    it("keeps the edit flow working after the dialog fix", async () => {
        listEmployeesMock
            ...
            .mockResolvedValueOnce(
                makeEnvelope([makeEmployee({ position: "Lead HR" })]),
            );

        const submittedUpdatePayloads = [];
        updateEmployeeMock.mockImplementationOnce(
            async (_api, _employeeId, payload) => {
        ...
        expect(submittedUpdatePayloads[0]).toEqual(
            expect.objectContaining({
                position: "Lead HR",
            }),
        );
    ...
    it("renders localized english labels for the employee page and form", async () => {
        ...
        expect(wrapper.text()).toContain(en["table.columns.position"]);
        expect(wrapper.text()).toContain(en["table.columns.company"]);

        await findButtonByText(wrapper, en["employees.new"]).trigger("click");
        await flushPromises();

        expect(wrapper.text()).toContain(en["table.columns.employee_number"]);
        expect(wrapper.text()).toContain(en["table.columns.name"]);
        expect(wrapper.text()).toContain(en["table.columns.email"]);
