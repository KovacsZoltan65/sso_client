...
describe('Welcome', () => {
    it('shows the SSO login entry point for guests', () => {
        ...
        expect(wrapper.text()).toContain('SSO bejelentkezes');
    ...
    it('shows dashboard and logout actions plus the active session summary for authenticated users', () => {
        ...
        expect(wrapper.text()).toContain('Kijelentkezes');
        expect(wrapper.text()).toContain('Aktiv session: SSO User (sso.user@example.test)');
