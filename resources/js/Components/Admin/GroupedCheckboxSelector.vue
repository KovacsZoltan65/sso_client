...
const groupedOptions = computed(() => {
    ...
    props.options.forEach((option) => {
        ...
        const rawValue = String(option.helper ?? optionValue);
        const [parsedResource = "misc", parsedAction = rawValue] = rawValue.split(".");
        const resource = option.groupKey ?? option.group ?? parsedResource;
        const groupLabel = option.groupLabel ?? prettifyResource(resource);
        const action = option.action ?? parsedAction;
        const itemLabel = option.itemLabel ?? prettifyAction(action === rawValue ? optionLabel : action);
        const searchableText = `${resource} ${groupLabel} ${action} ${optionLabel} ${rawValue}`.toLowerCase();
...
<template>
    <div class="grid min-w-0 gap-4 overflow-x-hidden">
        <div class="grid gap-2">
            <label for="grouped-checkbox-search" class="text-sm font-medium text-slate-700">{{ fieldLabel }}</label>
            <InputText
        ...
        <div
            :class="[
                'grid min-w-0 gap-4 overflow-x-hidden rounded-2xl border border-slate-200 bg-slate-50/80 p-4',
                allowInternalScroll ? 'max-h-[28rem] overflow-y-auto' : 'overflow-y-visible',
            ]"
        >
            <div
            ...
            >
                <div class="flex flex-col gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="text-xs text-slate-500">{{ group.items.length }} {{ groupCountLabel }}</p>
                    </div>
                ...
                >
                    ...
                    >
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="break-words text-sm font-medium text-slate-800">{{ item.label }}</div>
                            <div class="break-words text-xs leading-5 text-slate-500">{{ item.helper }}</div>
                        </div>
                    </label>
                </div>
            </div>

            <div v-if="groupedOptions.length === 0" class="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-8 text-center text-sm text-slate-500">
                {{ emptyMessage }}
            </div>
        </div>
    </div>
