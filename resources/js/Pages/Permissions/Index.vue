...
async function submitCreate() {
    ...
    try {
        ...
        toast.add({
            ...
            detail: "A permission letrehozasa sikeres volt.",
            life: 3000,
        });
        closeCreateDialog();
        tableState.page = 1;
...
async function submitUpdate() {
    ...
    try {
        ...
        toast.add({
            severity: "success",
            summary: "Sikeres muvelet",
            detail: "A permission frissitese sikeres volt.",
            life: 3000,
        });
        closeEditDialog();
        await loadPermissions();
        ...
        accept: async () => {
            try {
                ...
                toast.add({
                    severity: "success",
                    summary: "Sikeres muvelet",
                    detail: "A permission torlese sikeres volt.",
                    life: 3000,
                });
...
function permissionActionItems(permission) {
    return [
        props.permissions.update && permission.can?.update !== false
            ? {
                  label: "Szerkesztes",
                  icon: "pi pi-pencil",
                  command: () => openEditDialog(permission),
              }
            : null,
        props.permissions.delete && permission.can?.delete !== false
            ? {
                  label: "Torles",
                  icon: "pi pi-trash",
...
async function refreshPermissions() {
    ...
    toast.add({
        severity: "success",
        summary: "Sikeres muvelet",
        detail: "A permission lista frissult.",
        life: 2500,
    });
}
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="admin-table-page">
            <PageHeader
                title="Permissions"
                description="A helyi jogosultsag-katalogus teljes adminisztracioja, lokalis role-hozzarendelesekhez."
            />
                        ...
                        >
                            ...
                            <template #empty>
                                <div class="px-6 py-10">
                                    <EmptyStatePanel
                                        title="Nincs megjelenitheto permission"
                                        description="A jelenlegi szurok mellett nincs talalat. Modositsd a keresest vagy hozz letre uj permissiont."
                                        :tags="['Permissions', 'Local RBAC']"
                                    />
                                </div>
                            </template>
                        ...
                        <template v-else-if="items.length > 0">
                            ...
                            >
                                ...
                                <div class="mt-5 flex gap-3">
                                    <Button
                                        v-if="permissions.update && permission.can?.update !== false"
                                        label="Szerkesztes"
                                        severity="secondary"
                                        text
                                    ...
                                    />
                                    <Button
                                        v-if="permissions.delete && permission.can?.delete !== false"
                                        label="Torles"
                                        severity="danger"
                                        text
