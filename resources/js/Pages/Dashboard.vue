    ...
    <AuthenticatedLayout>
        ...
        <div class="space-y-6">
            <section class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <StatCard label="Users" :value="stats.users" icon="pi pi-users" />
                <StatCard label="Roles" :value="stats.roles" icon="pi pi-shield" />
                <StatCard label="Permissions" :value="stats.permissions" icon="pi pi-lock" />
                <StatCard label="Activity Entries" :value="stats.activityEntries" icon="pi pi-history" />
            </section>
                    ...
                    <div class="mt-5 space-y-4">
                        ...
                        >
                            ...
                            <p class="mt-2 text-xs uppercase tracking-[0.18em] text-slate-400">
                                {{ recentUser.roles.join(', ') || 'no assigned roles' }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <section class="shell-card p-6">
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    >
                        <p class="text-sm font-semibold text-slate-900">{{ entry.description }}</p>
                        <p class="mt-2 text-xs uppercase tracking-[0.18em] text-slate-400">{{ entry.event || 'event.pending' }}</p>
