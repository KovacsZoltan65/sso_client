...
const configuredServerPath = process.env.SSO_E2E_SERVER_PATH ?? path.join('..', 'sso_server');
...
const clientBaseUrl = process.env.SSO_E2E_CLIENT_BASE_URL ?? `http://127.0.0.1:${clientPort}`;
const serverBaseUrl = process.env.SSO_E2E_SERVER_BASE_URL ?? `http://127.0.0.1:${serverPort}`;

const clientDatabase = path.join(runtimeDir, 'client-e2e.sqlite');
const serverDatabase = path.join(runtimeDir, 'server-e2e.sqlite');
const sharedClientSecret = process.env.SSO_E2E_CLIENT_SECRET ?? 'e2e-client-secret';
